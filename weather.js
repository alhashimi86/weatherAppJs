...
async function getUser() {
  let url = `https://cors-anywhere.herokuapp.com/http://api.openweathermap.org/data/2.5/forecast?q=${city.value}&units=metric&APPID=1829fa4b735c5df44522c1a982e779fd`;
...
function bringData(data) {
  ...
  document.getElementById(
    "icon"
  ).innerHTML = `<img src='icons/${data.list[0].weather[0].icon}.png'>`;
  document.getElementById(
    "desc"
  ).innerHTML = `${data.list[0].weather[0].description}`;
  document.getElementById(
    "name"
  ).innerHTML = `${data.city.name},${data.city.country}`;
  ...
  ${days[date.getDay() + 1]}  ${Math.round(
    data.list[1].main.temp
  )}${String.fromCharCode(176)} C <br><br><br>
   ${days[date.getDay() + 2]} ${Math.round(
    data.list[2].main.temp
  )}${String.fromCharCode(176)} C<br><br><br>
    ${days[0]}     ${Math.round(data.list[3].main.temp)}${String.fromCharCode(
    176
  )} C
  </div>`;
}
...
function getImage() {
  ...
  fetch(url)
    ...
    .then(data => {
      ...
      document.getElementById(
        "main"
      ).style.backgroundImage = `url(${data.photos[0].image.mobile})`;
    });
}
let month = new Array();
month[0] = "January";
